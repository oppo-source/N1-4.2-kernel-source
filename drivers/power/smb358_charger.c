...
/*sjc2014-11-14 add*/
#define USE_LOWER_CHG_CURRENT	///1.5A
#define USE_LOWER_FLOAT_VOLTAGE	///4.25V
#define USE_NEWER_CHARGE_STANDARD //-10~0: 0.1C-4V; 0~10: 450mA-4.2V; 10~45: 1.5A-4.25V; 45~55: 0.3C-4.1V
#ifdef USE_LOWER_FLOAT_VOLTAGE
#undef CHG_COMPLETE_CHECK_COUNT
#undef CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__COOL
#undef CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__NORMAL
#undef CHG_COMPLETE_CHECK_VOLTAGE__NON_STANDARD__COOL
#undef CHG_COMPLETE_CHECK_VOLTAGE__NON_STANDARD__NORMAL
#undef CHG_COMPLETE_CHECK_VOLTAGE__USB__COOL
#undef CHG_COMPLETE_CHECK_VOLTAGE__USB__NORMAL
#ifndef USE_NEWER_CHARGE_STANDARD
#define CHG_COMPLETE_CHECK_COUNT									12
#define CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__COOL			4200
#define CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__NORMAL			4250
#define CHG_COMPLETE_CHECK_VOLTAGE__NON_STANDARD__COOL		4200
#define CHG_COMPLETE_CHECK_VOLTAGE__NON_STANDARD__NORMAL	4250
#define CHG_COMPLETE_CHECK_VOLTAGE__USB__COOL					4200
#define CHG_COMPLETE_CHECK_VOLTAGE__USB__NORMAL				4250
#else				
#define CHG_COMPLETE_CHECK_COUNT									12
#define CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__COOL			4200
#define CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__NORMAL			4250
...
//#undef BAT_TEMP__TEN
#undef STANDARD__FAST_CHG__COOL_CURRNET
#undef CHG_RESUME_CHECK_VOLTAGE__STANDARD__COOL
#undef CHG_RESUME_CHECK_VOLTAGE__STANDARD__NORMAL
#undef CHG_RESUME_CHECK_VOLTAGE__NON_STANDARD__COOL
#undef CHG_RESUME_CHECK_VOLTAGE__NON_STANDARD__NORMAL
#undef CHG_RESUME_CHECK_VOLTAGE__USB__COOL
#undef CHG_RESUME_CHECK_VOLTAGE__USB__NORMAL
#undef MAX_CHARGING_TIMEOUT_DCP

//#define BAT_TEMP__TEN												20
#define STANDARD__FAST_CHG__COOL_CURRNET						FAST_CHG_CURRENT__450MA
#define CHG_RESUME_CHECK_VOLTAGE__STANDARD__COOL				4100//mv
#define CHG_RESUME_CHECK_VOLTAGE__STANDARD__NORMAL			4150//mv
#define CHG_RESUME_CHECK_VOLTAGE__NON_STANDARD__COOL			4100//mv
#define CHG_RESUME_CHECK_VOLTAGE__NON_STANDARD__NORMAL		4150//mv
#define CHG_RESUME_CHECK_VOLTAGE__USB__COOL					4100//mv
#define CHG_RESUME_CHECK_VOLTAGE__USB__NORMAL					4150//mv
#define MAX_CHARGING_TIMEOUT_DCP									10*60*60//10hours
#endif //USE_NEWER_CHARGE_STANDARD
#endif //USE_LOWER_FLOAT_VOLTAGE
...
static int set_battery_capacity_full(bool set_value);//sjc1117
...
#ifdef USE_LOWER_CHG_CURRENT //sjc2014-11-14 Modify
	//Add code here?
#else
...
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc2014-11-14 Modify
...
static void smb358_charging_complete_check(struct smb358_charger *smb358_chg)
{
	...
	if ((smb358_battery_temperature_region_get(smb358_chg) == BATTERY_TEMP_REGION__WARM) && (i_chg < CHG_COMPLETE_CHECK__CURRENT * 2))
		counts++;
	else if (i_chg < CHG_COMPLETE_CHECK__CURRENT * 2)//use 200mA for more full
		counts++;
	else
		counts = 0;
	...
	if (smb358_bit_test(smb358_chg, IRQ_FLAGS__CHG_COMPLETE)){
		if (smb358_charging_current_get(smb358_chg) < CHG_COMPLETE_CHECK__CURRENT + 50) {//sjc0930 to avoid the condition that press power key for 7s
			if ((smb358_battery_temperature_get(smb358_chg) > BAT_TEMP__ZERO
					...
					&& smb358_battery_voltage_get(smb358_chg) > CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__COOL - 50)
					|| (smb358_battery_temperature_get(smb358_chg) >= BAT_TEMP__TEN
					&& smb358_battery_temperature_get(smb358_chg) < BAT_TEMP__FORTY_FIVE
					&& smb358_battery_voltage_get(smb358_chg) > CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__NORMAL - 50)
					|| (smb358_battery_temperature_get(smb358_chg) <= BAT_TEMP__ZERO  //sjc1117
					&& smb358_battery_temperature_get(smb358_chg) > BAT_TEMP__TEN_BELOW_ZERO
					&& smb358_battery_voltage_get(smb358_chg) > CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__LITTLE_COLD - 50)
					|| (smb358_battery_temperature_get(smb358_chg) >= BAT_TEMP__FORTY_FIVE
					&& smb358_battery_temperature_get(smb358_chg) < BAT_TEMP__FIFTY_FIVE
					&& smb358_battery_voltage_get(smb358_chg) > CHG_COMPLETE_CHECK_VOLTAGE__STANDARD__WARM - 50))
				smb358_charging_complete_request_set(smb358_chg, true);
		}
		//smb358_charging_complete_request_set(smb358_chg, true);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (smb358_battery_temperature_get(the_smb358_charger) > BAT_TEMP__ZERO
				...
				&& smb358_battery_capacity_get(the_smb358_charger) < 100) {
			val->intval = CHARGING_STATUS__CHARGING;
		} else {
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if ((smb358_battery_temperature_get(the_smb358_charger) > BAT_TEMP__ZERO)//sjc0813
			...
			if (smb358_charging_status_get(the_smb358_charger) == CHARGING_STATUS__FULL
					&& smb358_battery_capacity_get(the_smb358_charger) < 100) {
				set_battery_capacity_full(true);
			} else {
				set_battery_capacity_full(false);
			}
...
#ifdef USE_LOWER_CHG_CURRENT //sjc2014-11-14 Add
...
static void smb358_otg_enable(struct smb358_charger *smb358_chg)
{
	...
	if (ret) {
		...
		smb358_register_masked_write(smb358_chg->client, THERM_AND_SYSTEM_CONTROL_A_REGISTER_ADDR,
				SWITCH_FREQ_CONTROL__MASK, SWITCH_FREQ_CONTROL__1P5M);//sjc20140331 swich to 1.5MHz for otg, Qcom
	}
	else
...
static void smb358_otg_disable(struct smb358_charger *smb358_chg)
{
	...
	if (ret) {
		...
		smb358_register_masked_write(smb358_chg->client, THERM_AND_SYSTEM_CONTROL_A_REGISTER_ADDR,
				SWITCH_FREQ_CONTROL__MASK, SWITCH_FREQ_CONTROL__3M);//sjc20140331 switch back to 3MHz
	}
	else
...
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc2014-11-14 Modify
static int set_battery_capacity_full(bool set_value)
{
	if (batt_fuelgauge && batt_fuelgauge->set_full_capacity) {
		return batt_fuelgauge->set_full_capacity(set_value);
	} else {
...
/*
static bool get_battery_capacity_full(void)
{
	if (batt_fuelgauge && batt_fuelgauge->get_full_capacity) {
		return batt_fuelgauge->get_full_capacity();
	} else {
		pr_err("%s:Warning: batt_fuelgauge not ready!\n",__func__);
		return 0;
	}
