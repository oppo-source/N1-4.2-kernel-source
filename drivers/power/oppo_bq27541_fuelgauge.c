...
struct bq27541_device_info {
	...
	atomic_t suspended;
	bool set_full_flag;//sjc1117
};
static struct bq27541_device_info *bq27541_info;

#define USE_LOWER_FLOAT_VOLTAGE	//sjc20141124  4.25V
#define INVALID_SOC	110
extern void backup_soc_N1(int soc);
extern int read_soc_N1(void);
...
static unsigned int bq27541_capacity_calibrate(unsigned int batt_capacity)
{
	...
	int counter_temp = 0;//sjc20141124
	static int charging_pre = 0;
	...
	*/
	if(abs(batt_capacity - bq27541_info->batt_capacity_pre) >= BATT_CAPACITY_INVALID_LEVEL
			|| bq27541_info->batt_capacity_pre > 55 || bq27541_info->set_full_flag) {//sjc20141124
	...
	if (charging_now ^ charging_pre) {//sjc20141124
		charging_pre = charging_now;
		bq27541_info->capacity_saltate_counter = 0;
	}
	if (charging_now) {
		counter_temp = CAPACITY_SALTATE_COUNTER;
	} else {
		if (bq27541_info->batt_capacity_pre == 100)
			counter_temp = CAPACITY_SALTATE_COUNTER * 2;//* 6;///6mins
		else if (bq27541_info->batt_capacity_pre > 96)
			counter_temp = CAPACITY_SALTATE_COUNTER * 2;//* 5;///5mins
		else if (bq27541_info->batt_capacity_pre > 93)
			counter_temp = CAPACITY_SALTATE_COUNTER * 2;//* 4;///4mins
		else if (bq27541_info->batt_capacity_pre > 90)
			counter_temp = CAPACITY_SALTATE_COUNTER * 2;//* 3;///3mins
		else if (bq27541_info->batt_capacity_pre > 86)
			counter_temp = CAPACITY_SALTATE_COUNTER * 2;///2mins
		else if (bq27541_info->batt_capacity_pre > 83)
			counter_temp = CAPACITY_SALTATE_COUNTER * 1;///1mins
		else
			counter_temp = CAPACITY_SALTATE_COUNTER / 2;///0.5mins
	}
	if (bq27541_info->vbatt_mvolts > CAPACITY_SALTATE_CHANGE_MVOLTS){    
			if(bq27541_info->capacity_saltate_counter < counter_temp){
	...
	if(charging_now){
		if(batt_capacity > bq27541_info->batt_capacity_pre || bq27541_info->set_full_flag){//sjc1117
...
static int get_battery_capacity_fuelgague(void)
{
	...
	#endif	
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc20150112
...
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc20150112 for long press POWERKEY shuntdown
	if (backup_soc != bq27541_info->batt_capacity_pre) {
		backup_soc_N1(bq27541_info->batt_capacity_pre);
		backup_soc = bq27541_info->batt_capacity_pre;
	}
#endif
...
static int set_battery_full_capacity(bool set_value)//sjc1117
{
	if(fuelgague_is_present()){
		bq27541_info->set_full_flag = !!set_value;
		return bq27541_info->set_full_flag;
...
/*
static bool get_battery_full_capacity(void)//sjc1117
{
	if(fuelgague_is_present()){
		return !!bq27541_info->set_full_flag;
	}
				 ...
				 const struct i2c_device_id *id)
{
	...
	int err = 0;
	int retval = 0;
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc20141124
	int shutdown_soc = 0;
#endif
...
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc20141124
	shutdown_soc = read_soc_N1();
	if (shutdown_soc != INVALID_SOC) {
		if (bq27541_info->batt_capacity_pre > 50 && shutdown_soc > bq27541_info->batt_capacity_pre)
			bq27541_info->batt_capacity_pre = shutdown_soc - 1;
	}
#endif
	...
	bq27541_info->set_full_flag = false;//sjc1117
...
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc20141124
	backup_soc_N1(get_battery_capacity_fuelgague());
#endif
...
#ifdef USE_LOWER_FLOAT_VOLTAGE //sjc20141124		
		resume_soc = read_soc_N1();
		batt_soc = bq27541_info->batt_capacity_pre;//frome bq27541_update_capacity_pre_resume()
		if (resume_soc != INVALID_SOC && resume_soc > 50 && resume_soc > batt_soc) {
			if ((bq27541_info->rtc_resume_time - bq27541_info->rtc_suspend_time) >= RESUME_TIME * 90) {//90mins
				//DO Nothing but use default value
			} else if ((bq27541_info->rtc_resume_time - bq27541_info->rtc_suspend_time) >= RESUME_TIME * 60) {//60mins
				if (resume_soc - batt_soc > 6)
					bq27541_info->batt_capacity_pre = resume_soc - 6;
				else if (resume_soc - batt_soc > 4)
					bq27541_info->batt_capacity_pre = resume_soc - 4;
				else if (resume_soc - batt_soc > 2)
					bq27541_info->batt_capacity_pre = resume_soc - 2;
			} else if ((bq27541_info->rtc_resume_time - bq27541_info->rtc_suspend_time) >= RESUME_TIME * 40) {//40mins
				if (resume_soc - batt_soc > 5)
					bq27541_info->batt_capacity_pre = resume_soc - 5;
				else if (resume_soc - batt_soc > 3)
					bq27541_info->batt_capacity_pre = resume_soc - 3;
				else if (resume_soc - batt_soc > 2)
					bq27541_info->batt_capacity_pre = resume_soc - 2;
			} else if ((bq27541_info->rtc_resume_time - bq27541_info->rtc_suspend_time) >= RESUME_TIME * 30) {
				if (resume_soc - batt_soc > 4)
					bq27541_info->batt_capacity_pre = resume_soc - 4;
				else if (resume_soc - batt_soc > 2)
					bq27541_info->batt_capacity_pre = resume_soc - 2;
				else if (resume_soc - batt_soc > 1)
					bq27541_info->batt_capacity_pre = resume_soc - 1;
			} else if ((bq27541_info->rtc_resume_time - bq27541_info->rtc_suspend_time) >= RESUME_TIME * 20) {
				if (resume_soc - batt_soc > 3)
					bq27541_info->batt_capacity_pre = resume_soc - 3;
				else if (resume_soc - batt_soc > 1)
					bq27541_info->batt_capacity_pre = resume_soc - 1;
			} else if ((bq27541_info->rtc_resume_time - bq27541_info->rtc_suspend_time) >= RESUME_TIME * 10) {
				if (resume_soc - batt_soc > 3)
					bq27541_info->batt_capacity_pre = resume_soc - 2;
				else if (resume_soc - batt_soc > 1)
					bq27541_info->batt_capacity_pre = resume_soc - 1;
			} else {
				if (resume_soc - batt_soc > 1)
					bq27541_info->batt_capacity_pre = resume_soc - 1;
			}
		}
...
static void bq27541_battery_shutdown(struct i2c_client *client)//sjc20141124
{
...
static struct i2c_driver bq27541_battery_driver = {
	...
	.shutdown = bq27541_battery_shutdown,//sjc20141124
	.id_table = bq27541_id,
};
