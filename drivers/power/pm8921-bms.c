...
/* OPPO 2014-11-24 sjc Modify begin for N1 */
#ifdef CONFIG_VENDOR_EDIT
#define INVALID_SOC		110
void backup_soc_N1(int soc)
{
	if (the_chip == NULL || soc < 0 || soc > 100) {
		printk(KERN_ERR "===%s: chip or soc invalid, write INVALID_SOC===\n", __func__);
		pm8xxx_writeb(the_chip->dev->parent, TEMP_SOC_STORAGE, INVALID_SOC);
		return;
	} else {
...
/* return shutdown soc or error (110) */
int read_soc_N1(void)
{
	u8 soc = 0;
	int rc = 0;
	...
	rc = pm8xxx_readb(the_chip->dev->parent, TEMP_SOC_STORAGE, &soc);
	if (rc) {
	...
	if (soc == 0xff)
		...
		soc = INVALID_SOC;
printk(KERN_ERR "===%s: shutdown_soc[%d]===\n", __func__, soc);
	return soc;

}
#endif
/* OPPO 2014-11-24 sjc Modify end */

/* OPPO 2013-02-27 chendx Delete begin for not use IVAG VALUE and backup calib soc*/
void backup_calib_soc(int calib_soc)
{
	if (get_pcb_version() < PCB_VERSION_EVT_N1) {//sjc20141124 for Find5 only
		if (the_chip != NULL) {
...
module_init(pm8921_bms_init);//late_initcall(pm8921_bms_init);//sjc20141124
